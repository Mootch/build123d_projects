...
# %%
axle_r, axle_l = 6/2, 100
gap = 0.5
wheel_r, wheel_l, wheel_t = 15/2, 30, 1
thickness = 10
...
with BuildPart() as body:
    ...
    with Locations((axle_l/4, 0, 0), (-axle_l/4, 0, 0)) as wheel_locs:
...
with BuildPart() as grips:
    with BuildSketch(Plane.YZ) as grips_profile:
        ...
        chamfer(grips_profile.vertices().group_by(Axis.Y)[0].sort_by(Axis.X)[-1], thickness, angle=60)
        mirror(about=Plane.YZ)
    ...
    extrude(amount=1 * thickness, both=True)
    with BuildSketch(Plane.XZ) as cut_profile:
        with BuildLine() as cut_outline:
            l0 = Line((0,0), (0,thickness+gap))
            l1 = PolarLine(l0@1, 1 * thickness, -60, length_mode=LengthMode.VERTICAL)
